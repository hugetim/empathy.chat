...
def initialize_session(time_zone, user):
    ...
    with TimerLogger(
        "initialize_session", format="{name}: {elapsed:6.3f} s | {msg}"
    ) as timer:
    ...
    if starting_trust_level is None:
        notif_settings = {
            ...
            "email": {
                ...
                "eligible_users": [],
                "eligible_groups": [],
                "eligible_starred": True,
            },
            "sms": {
                "eligible": 0,
                "eligible_users": [],
                "eligible_groups": [],
                "eligible_starred": False,
            },
        }
        default_request = {
            "prop_time": dict(
                start_now=False,
                duration=port.DURATION_DEFAULT_MINUTES,
...
def _new_trust_level(user, starting_trust_level):
    ...
    def matched_with_distance1_member():
        ...
        for user2 in c.member_close_connections(user):
            user2_matches = app_tables.matches.search(
                users=[user, user2]
            )  # switching to exchange_log table
    ...
    if (
        (trust >= 3 and trust < 4)
        and user["url_confirmed_date"]
        and user["contributor"]
    ):
        trust = 4  # Partner
    return (
        starting_trust_level
        if (starting_trust_level and starting_trust_level >= trust)
        else trust
    )
...
def get_invite_from_port(port_invite):
    from . import groups, groups_server, invites, invites_server
...
def do_signup(email, port_invite):
    ...
    if newly_created:
        anvil.users.send_password_reset_email(
            email
        )  # This can also raise AuthenticationFailed, but shouldn't
    else:
        raise anvil.users.UserExists(
            f"An account already exists for this email address."
        )
    return user
...
def _emails_equal(a, b):
    a_local, a_domain = a.split("@")
    b_local, b_domain = b.split("@")

    if a_domain.lower() in ["gmail.com", "googlemail.com"]:
        a_local = a_local.replace(".", "")

    if b_domain.lower() in ["gmail.com", "googlemail.com"]:
        b_local = b_local.replace(".", "")

    return a_local.lower() == b_local.lower() and a_domain.lower() == b_domain.lower()
...
def remove_user(user):
    ...
    if user and (not user["init_date"]):
        user["enabled"] = False
        if (
            not user["trust_level"]
            and not user["confirmed_email"]
            and not user["time_zone"]
        ):
            user.delete()
...
def _check_phone_code(code, user):
    current_code_rows = app_tables.codes.search(
        order_by("date", ascending=False), user=user, type="phone"
    )
    any_failed = False
...
def _check_for_confirmed_invites(user):
    ...
    for invite_row in sm.inviteds(user):
        invite_reply = app_tables.invites.get(
            origin=False, user1=user, link_key=invite_row["link_key"], current=True
        )
        if invite_reply:
            ...
            if c.try_connect(invite_row, invite_reply):
                ...
                sm.add_invite_guess_fail_prompt(
                    invites_server.Invite.from_invite_row(invite_row, user=user)
                )
...
def save_name(name_item, user_id=""):
    ...
    user = sm.get_acting_user(user_id)
    user.update(
        first_name=name_item["first"].strip(), last_name=name_item["last"].strip()
    )
    matcher.propagate_update_needed(user)
...
def save_user_field(item_name, value, user_id=""):
    if item_name in ["how_empathy", "profile"]:
        ...
        sm.warning(
            f"Attempt to use 'save_user_field' for unauthorized item_name: {item_name}"
        )
...
def get_settings(user_id=""):
    ...
    for medium in ["sms", "email"]:
        ...
        elig_items[medium]["eligible_users"] = [
            sm.get_port_user(sm.get_other_user(u_id), user1=user, simple=True)
            for u_id in elig_items[medium]["eligible_users"]
        ]
        eligible_group_rows = [
            app_tables.groups.get_by_id(g_id)
            for g_id in elig_items[medium]["eligible_groups"]
        ]
        elig_items[medium]["eligible_groups"] = [
            groups.Group(group_row["name"], group_row.get_id())
            for group_row in eligible_group_rows
        ]
...
def set_notif_settings(notif_settings, elig_items, user_id=""):
    ...
    user = sm.get_acting_user(user_id)
    for medium in ["sms", "email"]:
        notif_settings[medium] = {
            k: elig_items[medium][k] for k in ["eligible", "eligible_starred"]
        }
        notif_settings[medium]["eligible_users"] = [
            port_user.user_id for port_user in elig_items[medium]["eligible_users"]
        ]
        notif_settings[medium]["eligible_groups"] = [
            group.group_id for group in elig_items[medium]["eligible_groups"]
        ]
...
def get_eligibility_specs(user):
    ...
    for medium in ["sms", "email"]:
        if notif_settings.get(medium):
            specs[medium] = {
                k: notif_settings[medium][k] for k in ["eligible", "eligible_starred"]
            }
            specs[medium]["user"] = user
            specs[medium]["eligible_users"] = [
                sm.get_other_user(u_id)
                for u_id in notif_settings[medium]["eligible_users"]
            ]
            specs[medium]["eligible_groups"] = [
                app_tables.groups.get_by_id(g_id)
                for g_id in notif_settings[medium]["eligible_groups"]
            ]
    return specs
