from anvil import *
from anvil.js import ExternalError, window
...
class MatchForm(MatchFormTemplate):
    ...
    def reset_jitsi(self):
        if self.jitsi_embed:
            ...
            Notification(
                "The previous video chat was just the waiting room. You have now entered the empathy chat room.",
                timeout=None,
                style="warning",
            ).show()
        self.add_jitsi_embed()
    ...
    def add_jitsi_embed(self):
        if not self.jitsi_embed:
            self.jitsi_embed = MyJitsi(
                item={
                    "room_name": self.item.jitsi_code,
                    "name": glob.name,
                    "domain": self.item.jitsi_domain,
                }
            )
    ...
    def init_slider_panel(self):
        slider_item = {
            "their_name": "",
            "status": self.item.slider_status,
    ...
    def init_subscriptions(self):
        ...
        glob.publisher.subscribe(
            "match.update_how", self, self.update_how_empathy_items
        )
    ...
    def base_status_reset(self):
        ...
        self.message_textbox.tooltip = (
            ""
            if matched
            else "You will be able to send a message once someone else has joined"
        )
    ...
    def pinged(self):
        with h.PausedTimer(self.timer_2):
            ...
            if self.jitsi_embed:
                with ui.BrowserTab(
                    "Someone waiting to join your empathy.chat",
                    "_/theme/favicon-dot.ico",
                ):
                    ready = confirm(
                        "Someone has asked to join your empathy chat. Are you still available to exchange empathy?"
                    )
    ...
    def process_pinged_response(self, ready):
        if ready:
            ...
            if self.item.status == "requesting":
                Notification(
                    "The user who had asked to join has now cancelled their request.",
                    timeout=None,
                    style="warning",
                ).show()
        else:
    ...
    def update_their_external(self, dispatch=None):
        if self.item.their_external:
            message = (
                f"{self.item.their_name} has left the empathy.chat window "
    ...
    def _update_doorbell_visible(self):
        self.mute_doorbell_link.visible = (
            self.status_label.visible and self._clips_loaded
        )

    def _update_doorbell_link(self):
        if self._doorbell_muted:
            self.mute_doorbell_link.icon = "fa:bell-slash"
            self.mute_doorbell_link.text = (
                "doorbell muted (click to play sound upon arrival)"
            )
        else:
            self.mute_doorbell_link.icon = "fa:bell"
            self.mute_doorbell_link.text = (
                "doorbell sound will play upon arrival (click to mute)"
            )
