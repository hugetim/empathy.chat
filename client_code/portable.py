...
class User(h.AttributeToKey):
    ...
    def distance_str(self):
        return (
            h.add_num_suffix(self.distance)
            if (self.distance is not None and self.distance < UNLINKED)
            else ""
        )
...
class UserFull(User):
    ...
    def name_item(self):
        return dict(
            key=self.name,
            value=self,
            subtext=self.distance_str_or_groups,
            title=self.first if self.first else self.name,
        )
    ...
    def url_confirmed_date_str(self):
        return (
            h.short_date_str(h.as_local_tz(self.url_confirmed_date))
            if self.url_confirmed_date
            else ""
        )
...
class UserProfile(UserFull):
    ...
    def profile_updated_date_str(self):
        return (
            h.short_date_str(h.as_local_tz(self.profile_updated_dt))
            if self.profile_updated_dt
            else ""
        )
...
class ProposalTime:
    def __init__(
        ...
        self.start_date = (
            start_date
            if (start_date or start_now)
            else h.round_up_datetime(
                h.now() + datetime.timedelta(minutes=DEFAULT_NEXT_MINUTES)
            )
        )
        ...
        if expire_date or start_now:
            ...
            cancel_buffer = (
                CANCEL_DEFAULT_MINUTES
                if not default_request
                else default_request["prop_time"]["cancel_buffer"]
            )
            self.expire_date = self.start_date - datetime.timedelta(
                minutes=cancel_buffer
            )
    ...
    def has_conflict(self, conflict_checks):
        ...
        if conflict_checks:
            ...
            for check_item in conflict_checks:
                if (
                    this["start"] < check_item["end"]
                    and check_item["start"] < this["end"]
                ):
                    return True
        return False
    ...
    def get_errors(self, conflict_checks=None):
        ...
        if not self.start_now:
            if self.start_date < now:
            ...
            elif self.start_date != self.expire_date and self.start_date < (
                now + datetime.timedelta(minutes=CANCEL_MIN_MINUTES)
            ):
                messages["start_date"] = (
                    "Start Time must be at least "
                    + str(CANCEL_MIN_MINUTES)
                    + " minutes away."
                )
            else:
                if self.expire_date < now:
                    messages[
                        "cancel_buffer"
                    ] = 'The specified "Cancel" time has already passed.'
                elif self.expire_date > self.start_date:
                    messages["cancel_buffer"] = (
                        'The "Cancel" time must be prior to the Start Time (by at least '
                        + str(CANCEL_MIN_MINUTES)
                        + " minutes)."
                    )
                elif self.start_date != self.expire_date and self.expire_date > (
                    self.start_date - datetime.timedelta(minutes=CANCEL_MIN_MINUTES)
                ):
                    messages["cancel_buffer"] = (
                        'The "Cancel" time must be at least '
                        + str(CANCEL_MIN_MINUTES)
        ...
        if self.has_conflict(conflict_checks):
            messages[
                "start_date"
            ] = "Time/duration overlaps with one of your existing requests."
        return messages
    ...
    def create_form_item(self):
        ...
        if self.start_now:
            ...
            cancel_buffer = round(
                (self.start_date - self.expire_date).total_seconds() / 60
            )
    ...
    def default_start():
        now = h.now()
        return {
            "s_min": now + datetime.timedelta(seconds=p.WAIT_SECONDS),
...
class Proposal:
    def __init__(
        ...
        if times is not None:
            ...
            self.times = [
                ProposalTime(start_now=default_request["prop_time"]["start_now"])
            ]
        else:
        ...
        if eligible is not None:
            ...
            self.eligible = (
                default_request["eligible"]["eligible"] if default_request else 0
            )
        if eligible_users is not None:
            ...
            self.eligible_users = (
                [glob.users[id] for id in default_request["eligible"]["eligible_users"]]
                if default_request
                else []
            )
        if eligible_groups is not None:
            ...
            self.eligible_groups = (
                [
                    nc.get_group_by_id(id)
                    for id in default_request["eligible"]["eligible_groups"]
                ]
                if default_request
                else []
            )
        if eligible_starred is not None:
            ...
            self.eligible_starred = (
                default_request["eligible"]["eligible_starred"]
                if default_request
                else True
            )
    ...
    def eligibility_desc(self):
        ...
        if self.eligible:
            desc = {
                1: "1st degree buddies",
                2: "buddies up to 2 degrees",
                3: "buddies up to 3 degrees",
            }
    ...
    def get_default_request_update(self):
        ...
        if not item["start_now"]:
            t0 = self.times[0]
            out["prop_time"]["cancel_buffer"] = round(
                (t0.start_date - t0.expire_date).total_seconds() / 60
            )
        out["eligible"] = {k: item[k] for k in ["eligible", "eligible_starred"]}
        out["eligible"]["eligible_users"] = [
            port_user.user_id for port_user in item["eligible_users"]
        ]
        out["eligible"]["eligible_groups"] = [
            group.group_id for group in item["eligible_groups"]
        ]
        return out
...
def default_cancel_date(now, start_date):
    minutes_prior = max(
        CANCEL_MIN_MINUTES,
        min(CANCEL_DEFAULT_MINUTES, ((start_date - now).total_seconds() / 60) / 2),
    )
...
def get_proposal_times_errors(now, proposal):
    ...
    if not proposal["start_now"]:
        if proposal["start_date"] < (
            ...
            messages["start_date"] = (
                "The Start Time must be at least "
                + str(CANCEL_MIN_MINUTES)
                + " minutes away."
            )
        else:
            cancel_date = proposal["start_date"] - datetime.timedelta(
                minutes=proposal["cancel_buffer"]
            )
            if cancel_date < now:
                messages[
                    "cancel_buffer"
                ...
                proposal["start_date"] - datetime.timedelta(minutes=CANCEL_MIN_MINUTES)
            ):
                messages["cancel_buffer"] = (
                    'The "Cancel" time must be at least '
                    + str(CANCEL_MIN_MINUTES)
